...
observation - 264 of its 296 floats, the private tail having been split off first - into
`(B, n_hist × (k_rows + 1), 8)` tokens whose position is a `(time, level)`
...
```mermaid
flowchart TD
    OBS["observation, 296 floats"] --> SPLIT["split_private"]
