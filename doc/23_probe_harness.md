...
|---|---|---|
| `raw` | The observation itself, all 224 floats | The floor. Every latent is a *function* of this vector |
...
The grid reaches `1e7` deliberately. `raw` is 224 standardised features, and a short corpus gives a
...
the total mass. Read it against `width` — absolute values are not comparable between a 224-float
