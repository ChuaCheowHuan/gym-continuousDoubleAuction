...
"""
#https://neuro.cs.ut.ee/the-use-of-embeddings-in-openai-five/
"If the number of actions is N and action embedding size is E then all action
embeddings can be represented as NxE matrix. On particular timestep you have M
available actions, so the full NxE embedding matrix is filtered to contain only
M rows, so now it is MxE. Output from LSTM FC must have dimensions Ex1
(actually there is probably also batch and timestep dimension,
but we can ignore those for now). Matrix multiplication between MxE and Ex1
produces Mx1 matrix. Softmax is taken over those M action scores and converts
those into probabilities." - Tambet Matiisen
"""
...
class ParametricActionCartpole(gym.Env):
    ...
    def __init__(self, max_avail_actions):
        ...
        self.observation_space = Dict({"action_mask": Box(0, 1, shape=(max_avail_actions, )),
...
class ParametricActionsModel(DistributionalQModel, TFModelV2):
    ...
    def __init__(self,
        ...
        super(ParametricActionsModel, self).__init__(obs_space,
                                                     ...
                                                     name,
                                                     **kw)
        self.action_embed_model = FullyConnectedNetwork(Box(-1, 1, shape=true_obs_shape),
                                                        action_space,
                                                        action_embed_size,
                                                        model_config,
                                                        name + "_action_embed")
...
if __name__ == "__main__":
    ...
    if args.run == "DQN":
        cfg = {# TODO(ekl) we need to set these to prevent the masked values
