...
class TestAccounting:
    ...
    def test_limit_order_placement_hold(self):
        ...
        # Verification
        assert self.trader_A.acc.cash == Decimal(900), "Cash should decrease by order value"
        assert self.trader_A.acc.cash_on_hold == Decimal(100), "Cash on hold should increase by order value"
        assert self.trader_A.acc.nav == Decimal(1000), "NAV should remain constant"
        ...
        # Verification
        assert self.trader_B.acc.cash == Decimal(898), "Cash should decrease by margin required (102)"
        assert self.trader_B.acc.cash_on_hold == Decimal(102), "Cash on hold should increase by 102"
        assert self.trader_B.acc.nav == Decimal(1000), "NAV should remain constant"
    ...
    def test_limit_order_cancellation(self):
        ...
        # Verification
        assert self.trader_A.acc.cash == Decimal(1000), "Cash should return to original"
        assert self.trader_A.acc.cash_on_hold == Decimal(0), "Cash on hold should be zero"
        assert self.trader_A.acc.cal_nav() == Decimal(1000), "NAV should remain constant"
        ...
        # Verification
        assert self.trader_B.acc.cash == Decimal(1000), "Cash should return to original"
        assert self.trader_B.acc.cash_on_hold == Decimal(0), "Cash on hold should be zero"
        assert self.trader_B.acc.cal_nav() == Decimal(1000), "NAV should remain constant"
    ...
    def test_market_short_matching(self):
        ...
        assert self.trader_A.acc.cash_on_hold == Decimal(0), "Hold released"
        assert self.trader_A.acc.position_val == Decimal(100), "Position value set"
        assert self.trader_A.acc.net_position == 1, "Net long position"
        assert self.trader_A.acc.cash == Decimal(900), "Cash remains decreased"
        assert self.trader_A.acc.cal_nav() == Decimal(1000), "NAV must apply: 900+0+100=1000"

        # --- Verify Agent B (Short, Aggressor) ---
        assert self.trader_B.acc.cash == Decimal(900), "Cash paid immediately"
        assert self.trader_B.acc.position_val == Decimal(100), "Short position value"
        assert self.trader_B.acc.net_position == -1, "Net short position"
        assert self.trader_B.acc.cal_nav() == Decimal(1000), "NAV must apply: 900+0+100=1000"
    ...
    def test_market_long_matching(self):
        ...
        assert self.trader_A.acc.cash_on_hold == Decimal(0), "Hold released"
        assert self.trader_A.acc.position_val == Decimal(100), "Short position value"
        assert self.trader_A.acc.net_position == -1, "Net short position"
        assert self.trader_A.acc.cal_nav() == Decimal(1000), "NAV must apply: 900+0+100=1000"

        # --- Verify Agent B (Long, Aggressor) ---
        assert self.trader_B.acc.cash == Decimal(900), "Cash paid"
        assert self.trader_B.acc.position_val == Decimal(100), "Long position value"
        assert self.trader_B.acc.net_position == 1, "Net long position"
        assert self.trader_B.acc.cal_nav() == Decimal(1000), "NAV must apply: 900+0+100=1000"
    ...
    def test_partial_fill(self):
        ...
        assert self.trader_A.acc.cash == Decimal(800)
        assert self.trader_A.acc.cash_on_hold == Decimal(100)
        assert self.trader_A.acc.position_val == Decimal(100)
        assert self.trader_A.acc.net_position == 1
        assert self.trader_A.acc.cal_nav() == Decimal(1000), "NAV must apply"
    ...
    def test_mark_to_market_long(self):
        ...
        assert self.trader_A.acc.nav == Decimal(1010), "NAV should increase"
        assert self.trader_A.acc.profit == Decimal(10), "Profit should be 10"
        ...
        assert self.trader_A.acc.nav == Decimal(990), "NAV should reflect loss"
    ...
    def test_mark_to_market_short(self):
        ...
        assert self.trader_B.acc.nav == Decimal(990), "NAV should decrease on price rise"
    ...
    def test_market_order_empty_book(self):
        ...
        assert self.trader_A.acc.cash == Decimal(1000)
        assert len(self.order_book.bids) == 0
        assert len(self.order_book.tape) == 0
        assert self.trader_A.acc.cal_nav() == Decimal(1000), "NAV must remain constant"
    ...
    def test_position_flip_long_to_short_aggressor(self):
        ...
        # Verify A: Net -1
        assert self.trader_A.acc.net_position == -1
        assert self.trader_A.acc.position_val == Decimal(100), "Value of new short"
        # Cash flow:
        # Started 900.
        # Sold 1 (closing long): +100 cash. Cash=1000.
        # Sold 1 (opening short): -100 cash (margin). Cash=900.
        assert self.trader_A.acc.cash == Decimal(900)
        assert self.trader_A.acc.cal_nav() == Decimal(1000), "NAV must remain constant"
    ...
    def test_position_flip_short_to_long_aggressor(self):
        ...
        # Verify A: Net +1
        assert self.trader_A.acc.net_position == 1
        assert self.trader_A.acc.position_val == Decimal(100), "Value of new long"
    ...
    def test_position_flip_long_to_short_passive(self):
        ...
        assert self.trader_A.acc.cash == Decimal(700), "Cash locked for limit sell"
        assert self.trader_A.acc.cash_on_hold == Decimal(200), "Hold increased"
        ...
        assert self.trader_A.acc.net_position == -1
        assert self.trader_A.acc.position_val == Decimal(100)
        assert self.trader_A.acc.cash == Decimal(900), "Cash returned after trade"
        assert self.trader_A.acc.cal_nav() == Decimal(1000), "NAV must remain constant"
    ...
    def test_position_flip_short_to_long_passive(self):
        ...
        # Check Intermediate
        assert self.trader_A.acc.cash == Decimal(700)
        assert self.trader_A.acc.cash_on_hold == Decimal(200)
        ...
        assert self.trader_A.acc.position_val == Decimal(100)
        assert self.trader_A.acc.cash == Decimal(900)
        assert self.trader_A.acc.cal_nav() == Decimal(1000)
